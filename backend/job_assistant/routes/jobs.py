...
def _llm_structured_resume(profile: dict[str, Any], job: dict[str, Any], user_id: int) -> dict[str, Any]:
    ...
    context = {
        "resume_text": profile.get("cv_text") or "", "profile": profile,
        "job": {
    ...
    structure = ai_orchestrator.ask_json(system, json.dumps(context), fallback, user_id=user_id, task_type="resume_document", workspace_id=job.get("workspace_id"))
...
def _llm_tailored_resume(profile: dict[str, Any], job: dict[str, Any], user_id: int) -> dict[str, Any]:
    ...
    tailored = ai_orchestrator.ask_json(system, json.dumps(context), fallback, user_id=user_id, task_type="resume_tailoring", workspace_id=job.get("workspace_id"))
