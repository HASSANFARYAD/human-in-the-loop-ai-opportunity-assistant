...
GMAIL_CONFIG_SERVICE_NAME = "gmail"


def _oauth_settings(user_id: int) -> dict:
    settings = get_integration_settings(user_id, GMAIL_CONFIG_SERVICE_NAME)
    config = settings.get("config") or {}
    if config.get("is_active") is False:
        raise RuntimeError("Gmail OAuth configuration is inactive.")
    ...
    scopes = config.get("scopes") or SCOPES
    if isinstance(scopes, str):
        scopes = [scope.strip() for scope in scopes.replace(",", " ").split() if scope.strip()]
    if not client_id or not client_secret or not redirect_uri:
        raise RuntimeError("Gmail OAuth is missing database configuration for client_id, client_secret, or redirect_uri.")
    return {
        "client_id": client_id,
        "client_secret": client_secret,
        "redirect_uri": redirect_uri,
        "scopes": scopes or SCOPES,
        "project_id": config.get("project_id") or "job-assistant",
    }


def _client_config(user_id: int) -> dict:
    settings = _oauth_settings(user_id)
    return {
        "web": {
...
def _flow(user_id: int, redirect_uri: str | None = None):
    from google_auth_oauthlib.flow import Flow

    oauth = _oauth_settings(user_id)
    config = _client_config(user_id)
...
def build_gmail_authorization_url(user_id: int, redirect_uri: str | None = None) -> str:
    ...
    flow = _flow(user_id, redirect_uri)
...
def exchange_gmail_code(user_id: int, code: str, redirect_uri: str | None, state: str) -> None:
    ...
    scopes = _oauth_settings(user_id)["scopes"]
    save_integration_settings(
        user_id,
        GMAIL_SERVICE_NAME,
        api_key="",
...
def get_gmail_connection(user_id: int) -> dict:
    ...
    oauth_config = get_integration_settings(user_id, GMAIL_CONFIG_SERVICE_NAME)
...
def _credentials_for_user(user_id: int):
    ...
    scopes = config.get("scopes") or _oauth_settings(user_id)["scopes"]
...
def _get_service(user_id: int | None = None):
    ...
    if user_id is None:
        raise RuntimeError("Gmail requires an authenticated user and database-backed OAuth configuration.")
...
def fetch_job_alert_messages(
    ...
    for msg in messages:
        ...
        out.append({
            "message_id": full.get("id", msg["id"]),
            "thread_id": full.get("threadId", ""),
            "subject": headers.get("subject", ""),
            "from": headers.get("from", ""),
            "date_received": date_received,
            "snippet": full.get("snippet", ""),
            "open_url": f"https://mail.google.com/mail/u/0/#inbox/{full.get('threadId') or full.get('id', msg['id'])}",
