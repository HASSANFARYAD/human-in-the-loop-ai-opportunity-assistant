...
export function AIWorkspaceView() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card><CardHeader><CardTitle>Prompt Versions</CardTitle></CardHeader><CardContent><DataTable rows={(prompts.data ?? []) as unknown as Record<string, unknown>[]} columns={["name", "version", "status", "updated_at"]} /></CardContent></Card>
      <Card><CardHeader><CardTitle>AI Activity Timeline</CardTitle></CardHeader><CardContent className="space-y-3">{(generations.data ?? []).length ? (generations.data ?? []).map((item, index) => <div key={item.id ?? index} className="flex gap-3 rounded-md border p-3"><Bot className="mt-0.5 h-4 w-4 text-primary" /><div className="grid gap-1 text-sm">{Object.entries((item ?? {}) as unknown as Record<string, unknown>).map(([key, value]) => <div key={key}><span className="font-medium">{key.replace(/_/g, " ")}:</span> {formatDisplayValue(value)}</div>)}</div></div>) : <div className="text-sm text-muted-foreground">No data available</div>}</CardContent></Card>
    </div>
  );
}
