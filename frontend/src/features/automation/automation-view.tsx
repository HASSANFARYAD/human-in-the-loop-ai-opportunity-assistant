  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Rules, scheduled jobs, approvals, recent runs, failures, and success rates.</p>
      </div>
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <CardContent className="p-5">
            <Workflow className="mb-3 h-5 w-5 text-primary" />
            <div className="text-2xl font-semibold">{rules.data?.length ?? 0}</div>
        ...
        <Card>
          <CardContent className="p-5">
            ...
            <div className="text-2xl font-semibold">{(runs.data ?? []).filter((r) => r.status === "success").length}</div>
            <div className="text-sm text-muted-foreground">Successful runs</div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-5">
            <AlertTriangle className="mb-3 h-5 w-5 text-warning" />
            <div className="text-2xl font-semibold">{errors.data?.length ?? 0}</div>
        ...
        <Card>
          <CardContent className="p-5">
            ...
            <div className="text-2xl font-semibold">{runs.data?.length ?? 0}</div>
      ...
      {activeTab === "runs" ? (
            ...
            {(runs.data ?? []).map((run) => (
              <div key={run.id} className="rounded-md border p-3">
                ...
                <div className="text-sm text-muted-foreground">
                  {run.trigger_event || "manual"} · {formatDate(run.started_at)}
                </div>
              </div>
            ))}
          </CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Failures and Activity</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {(errors.data ?? []).map((run) => (
              <div key={run.id} className="rounded-md border border-destructive/30 p-3">
                <div className="flex justify-between">
                  <span className="font-medium">Failure #{run.id}</span>
                  <Badge>{run.status}</Badge>
                </div>
                <div className="text-sm text-muted-foreground">{run.error || run.trigger_event || "No error message recorded"}</div>
              </div>
            ))}
          </CardContent>
        </Card>
