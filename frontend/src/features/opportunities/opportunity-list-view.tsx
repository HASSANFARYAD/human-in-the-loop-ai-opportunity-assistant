                ...
                <motion.tr key={item.id} variants={staggerRow} className="border-t transition-colors hover:bg-muted/30">
                  ...
                  <td className="p-3 text-right">
                    <div className="flex justify-end gap-2">
                      {item.url && importable ? <Button asChild size="sm" variant="outline"><a href={item.url} target="_blank" rel="noopener noreferrer"><ExternalLink className="h-3.5 w-3.5" /> Apply</a></Button> : <Button size="sm" variant="outline" disabled>Apply unavailable</Button>}
...
function RemindersView() {
  ...
  return (
    <div className="space-y-5">
      ...
      <Card><CardContent className="p-5"><DataTable rows={(reminders.data ?? []) as unknown as Record<string, unknown>[]} columns={["title", "due_at", "status", "job_id"]} /></CardContent></Card>
    </div>
  );
}
