...
export function OpportunityDetailView() {
  ...
  const generate = useMutation({ mutationFn: () => opportunityService.generateMaterials(id), onSuccess: (data) => { if (data?.ai_error) toast.warning(data.ai_error); else toast.success("Materials generated"); qc.invalidateQueries({ queryKey: ["materials", id] }); qc.invalidateQueries({ queryKey: ["ai-usage"] }); }, onError: (error) => toast.error(error.message) });
  const tailorResume = useMutation({ mutationFn: () => opportunityService.tailorResume(id, selectedProfileId), onSuccess: (data) => { if (data?.ai_error) toast.warning(data.ai_error); else toast.success("Tailored resume generated"); qc.invalidateQueries({ queryKey: ["resume-reviews", id] }); qc.invalidateQueries({ queryKey: ["ai-usage"] }); }, onError: (error) => toast.error(tailorResumeErrorMessage(error)) });
